...
import { handleNewMessage } from "@/features/chat/events/events";
...
const handlers: Record<string, EventHandler> = {
  new_message: (payload) =>
    handleNewMessage(payload as Parameters<typeof handleNewMessage>[0]),
...
export function dispatchWebSocketEvent(message: WebSocketMessage) {
  const handler = handlers[message.type];

  if (!handler) {
  console.warn("Unhandled websocket event:", message);
  return;
}
